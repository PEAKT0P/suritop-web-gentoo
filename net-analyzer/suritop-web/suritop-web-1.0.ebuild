...
try:
    ...
    print(f\"{geo.get('lat', 0)},{geo.get('lon', 0)}\")
except: print('0,0')
" 2>/dev/null)
	SERVER_LAT="${SERVER_LOC%%,*}"
	SERVER_LON="${SERVER_LOC##*,}"

	einfo "  Server location: ${SERVER_LAT}, ${SERVER_LON}"

	DB_PASS=$(python3 -c "
import configparser
c = configparser.ConfigParser()
c.read('${EROOT}/etc/suritop-web/suritop.conf')
print(c.get('Database', 'pass_r', fallback=''))
" 2>/dev/null)
...
PHPCONFIG
	...
	cat > "${EROOT}/var/www/suritop-web/htdocs/attackmap/config.php" << PHPCONFIG2
<?php
define('STATS_DB_HOST', 'localhost');
define('STATS_DB_NAME', 'server_stats');
define('STATS_DB_USER', 'stats_reader');
define('STATS_DB_PASS', '${DB_PASS}');
define('OUR_IP', '${DEFAULT_IP}');
...
PHPCONFIG2
	...
	einfo "Config files generated: suritop.conf + PHP configs"
